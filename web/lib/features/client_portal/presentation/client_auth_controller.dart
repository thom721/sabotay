...
/// mais pour l'identité Client — les deux contrôleurs partagent le même
/// `tokenStorageProvider` (un seul slot de token pour staff/client, voir
/// `super_admin_token_storage.dart`) : `client_login_screen.dart` et le
/// `LoginScreen` staff sont responsables d'appeler `clearSilently()` sur
/// l'autre contrôleur après une connexion réussie, pour éviter qu'un ancien
/// état ne reste en cache.
class ClientAuthController extends AsyncNotifier<ClientPrincipal?> {
  ...
  /// Appelé quand l'identité staff vient de se connecter avec succès sur le
  /// même navigateur — le token du Client (s'il y en avait un) est déjà
  /// remplacé, on ne fait qu'oublier l'état en mémoire, sans re-toucher au
  /// token.
  void clearSilently() {
    state = const AsyncData(null);
  }
