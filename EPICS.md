...
- `backend/server_main.py` créé, calqué sur `pos_api/server_main.py` : imports forcés des packages chargés dynamiquement (`psycopg`, `aiosqlite`, `passlib.handlers.bcrypt`, `jose`, `cryptography`, `multipart`, `aiosmtplib`, `dotenv`, `alembic` — **`twilio` volontairement retiré de cette liste depuis**, voir plus bas), `_fix_workdir()`, log de crash (`%PROGRAMDATA%/SabotayPro` / `~/Library/Application Support/SabotayPro`), popup d'erreur Windows.
...
- **Optimisation trouvée au premier vrai run CI (tag `v0.3.1`) et corrigée** : `Backend — Windows` dépassait déjà 57 min sans être terminé, alors que le job équivalent de pos_api (dépôt pourtant plus volumineux) ne dépasse jamais 48 min. Cause identifiée : `twilio` (SDK SMS) — absent des dépendances de pos_api — pèse 630 fichiers/~26 Mo, et `twilio.rest.Client` importe automatiquement tous ses modules produits au chargement, forçant Nuitka à compiler l'ensemble même si `core/notifications.py::send_sms` n'utilise qu'un seul appel (import déjà différé dans la fonction, ce qui n'aide pas Nuitka — son analyse statique suit les imports où qu'ils soient dans le code). Twilio étant déjà un canal secondaire depuis l'Epic 16 (SMS jamais configuré en prod, "mot de passe oublié" par email), exclu du binaire compilé : `--nofollow-import-to=twilio` ajouté aux deux jobs Nuitka (`build.yml`), `import twilio` retiré de `server_main.py`, repli gracieux ajouté dans `send_sms()` (`ModuleNotFoundError` attrapée, log au lieu d'un crash si Twilio était reconfiguré malgré tout sur un poste bureau). **Uniquement le binaire bureau compilé est concerné** — le cloud Docker tourne depuis les sources, jamais via Nuitka, SMS toujours pleinement fonctionnel là où c'est déjà configuré. Pas encore vérifié sur un nouveau run CI (le run `v0.3.1`, lancé avant ce correctif, continue avec l'ancienne configuration).
...
- **Bug réel trouvé et corrigé grâce au premier vrai run CI** (secrets configurés par l'utilisateur, tag `v0.3.0`) : deux blocs de commentaires dans la section `[Code]` (lignes 161-165 et 175-183) utilisaient `;` au lieu de `//` — syntaxe INI valide dans `[Setup]`/`[Files]`/etc., mais invalide en Pascal Script (`;` y termine une instruction, pas un commentaire). `ISCC.exe` échouait avec `'BEGIN' expected` à la première occurrence. Confirmé en comparant à `pos_api/certificat/pos-server.iss`, qui n'utilise jamais `;` dans sa propre section `[Code]`. Corrigé aux deux endroits ; **toujours pas compile-testé localement** (`ISCC.exe` reste indisponible sur macOS) — validation réelle au prochain tag/run CI.
...
- Recherche préalable : pos_api n'a qu'un seul thème (`pos_api/frontend/lib/core/theme.dart`), pas de mode sombre, palette `primary #0077C5`/`primaryDark #005A9C`, `accent #2CA01C`, `background #F0F2F5`, `surface #FFFFFF`, `textPrimary #1A202C`, `textSecondary #718096`, `divider #E2E8F0`, `error #E53E3E`, `warning #D69E2E`, `success #38A169`, `info #3182CE`, plus des couleurs de statut (`statusPaid`/`statusPartial`/`statusUnpaid`/`statusPending`).
- **Première passe incomplète, corrigée dans la même session après retour de l'utilisateur** : la première tentative n'avait repris que les *valeurs de couleur*, sans les propriétés structurelles du thème pos_api (rayons, paddings, bordures de champs, hauteur d'AppBar, etc.) — l'essentiel de "l'apparence" pos_api restait donc absent malgré une palette correcte. Repris entièrement `mobile/lib/core/theme/app_theme.dart::_build()` pièce par pièce contre `pos_api/frontend/lib/core/theme.dart::AppTheme.light` :
  - Carte : rayon 16→**12**, `margin: EdgeInsets.zero` ajouté.
  - AppBar : `toolbarHeight: 64`, `scrolledUnderElevation: 1`, `surfaceTintColor: transparent` ajoutés (absents avant).
  - Champs de saisie : fond passé de `surfaceVariant` à **`surface`** (blanc) + bordures explicites colorées (`divider`/`primary`/`error`) — remplace l'ancienne distinction par teinte de fond (Epic 4) par une distinction par bordure, comme pos_api.
  - Bouton plein : `elevation: 0` et `padding` (24/14) ajoutés.
  - Bouton contour (`OutlinedButtonThemeData`) : **absent jusqu'ici**, ajouté entièrement (bordure/texte `primary`, rayon 8).
  - Chips : forme pilule (rayon 999) → **rectangle arrondi (rayon 6)**, comme pos_api.
  - Texte : tailles explicites de pos_api reprises sur les styles qu'il définit lui-même (`displayLarge` 32/700, `displayMedium` 24/700, `titleLarge` 18/600, `bodyLarge` 15, `bodySmall` en couleur atténuée) — les styles que pos_api ne touche pas (headline*, `displaySmall`, `titleSmall`, `labelMedium/Small`) gardent l'échelle Material 3 déjà en place, pour limiter le risque de régression visuelle sur le reste de l'app (non vérifiable sans capture d'écran de chaque écran).
- Deux couleurs manquées à la première passe, ajoutées : `primaryDark` (`#005A9C`, distinct de `navyDark` qui reste une adaptation pour le mode sombre) et les couleurs de statut (`warning`, `statusPaid/Partial/Unpaid/Pending`), exposées même si pas encore raccrochées à un rôle précis du `ColorScheme` (4 rôles Material — primary/secondary/tertiary/error — ne suffisent pas à porter 5 concepts de couleur).
- `mobile/lib/core/theme/app_colors.dart` : **noms de constantes conservés à l'identique** (`navy`, `emerald`, `crimson`, `slate`, `lightBg`, etc.) pour ne casser aucun point d'usage dans l'app, seules les valeurs changent. `slate` (bleu "info", `#3182CE`) reprend exactement la couleur déjà utilisée côté web pour la carte "Clients actifs" du tableau de bord (Epic 15) — cohérence de palette entre web et mobile même si le web garde son doré (`gold`) comme couleur de marque principale pour les boutons (Epic 14, non touché ici, pas demandé).
- pos_api n'ayant pas de mode sombre, les variantes `*Dark` (utilisées quand `Brightness.dark`) restent **adaptées, pas copiées** : `navyDark` reprend `sidebarSelected` (`#2563EB`, la teinte que pos_api utilise lui-même pour ressortir sur son sidebar sombre — choisie plutôt que `primaryDark`, trop proche du fond sur un écran presque noir), `emeraldDark` reprend `success` (`#38A169`), `crimsonDark` est éclairci (`#FF6B6B`, pos_api n'a pas d'équivalent). Les neutres du mode sombre (fond/surface/texte, propres à SabotayPro, sans équivalent pos_api) restent inchangés.
- `flutter analyze` 0 erreur, `flutter build apk --debug` propre, **installé et lancé réellement sur un appareil physique** (WTCELERO5G) pour vérification visuelle après la correction.
...
## Epic 21 — Isolation multi-tenant au changement de compte (mobile) ✅ Terminé

**Objectif** : question explicite de l'utilisateur — si un agent d'une entreprise A se connecte sur l'app mobile, se déconnecte, puis un agent d'une entreprise B différente se connecte sur le **même appareil sans que l'app soit tuée entre-temps**, que se passe-t-il ?

**Recherche préalable (avant tout correctif)** : le backend est déjà solide — `TenantId` (`deps.py`) dérive `entreprise_id` exclusivement du JWT courant, toutes les requêtes SQL filtrent dessus (`compte_sabotay.py::get_by_id` vérifié explicitement). Aucune fuite de données possible côté serveur. **Deux trous réels trouvés côté client**, absents côté backend :
- Deux providers Riverpod globaux, non invalidés au changement de compte : `entrepriseProfilProvider` (profil affiché sur les reçus) et `clientListControllerProvider` (liste de clients) — restaient en mémoire avec les données de l'entreprise A jusqu'à un rafraîchissement manuel une fois l'agent B connecté. Le portail Client avait déjà résolu ce problème pour lui-même (`ClientAuthController.switchEntreprise` invalide 4 providers) — jamais répliqué côté staff/agent.
- La file d'attente hors-ligne (`offline_queue_service.dart`, `SharedPreferences`, une seule clé globale, aucun tag entreprise/utilisateur) : une collecte de l'agent A en attente d'envoi au moment où B se connecte aurait été rejouée avec le token de B — rejetée par le backend (isolation tenant), mais **perdue silencieusement** pour A, avec un message d'erreur générique affiché à B sans rapport avec lui.

**Premier correctif proposé, puis durci sur retour de l'utilisateur** : la première version vidait silencieusement la file hors-ligne en cas de changement d'entreprise — jugé trop risqué (perte réelle d'une collecte jamais transmise au serveur). Remplacé par un **blocage strict** de la connexion tant que la file n'est pas vide :
- `AuthController.login()` (`mobile/lib/features/auth/presentation/auth_controller.dart`) : capture l'utilisateur précédent **et son token** avant toute mutation. Si une file est en attente, tente d'abord de la vider (`drain`) tant que l'ancien token est encore actif — souvent suffisant à lui seul. Une fois le nouvel utilisateur récupéré, si son `entrepriseId` diffère de celui de l'agent précédent **et** qu'il reste des opérations en attente : **restaure l'ancien token** (l'agent précédent reste connecté, peut synchroniser dès qu'il retrouve le réseau) et fait échouer la connexion avec un message explicite (`SyncEnAttenteException`), plutôt que de perdre les opérations ou de les rejouer sous le mauvais compte.
- File vide (déjà à jour, ou vidée par le `drain` préalable) : la connexion se termine normalement, avec `ref.invalidate()` sur les deux providers scoped-entreprise (profil entreprise, liste de clients) pour éviter d'afficher les données de l'ancien compte.
- `login_screen.dart` : affiche le message de `SyncEnAttenteException` tel quel au lieu du message générique "mot de passe incorrect" utilisé pour toute autre erreur de connexion.
- Comportement inchangé si le même agent se reconnecte (même `entrepriseId`) — jamais de blocage ni de vidage dans ce cas.
- `flutter analyze` (mobile) 0 erreur, `flutter build apk --debug` propre, installé sur appareil physique.

**Non couvert, hors scope de la question posée** : le token lui-même était déjà correctement remplacé/effacé à chaque login/logout (`flutter_secure_storage`, pas de résidu). Les providers `.family` (soldes de compte, transactions par compte) sont déjà sans risque — clés par UUID, pas de collision plausible entre deux entreprises.

---

## Epic 6 — Offline-first mobile complet ✅ Terminé

**Objectif** : explicitement hors-scope du MVP par le PRD (§5.2, §9, §13 Phase 2), mais **confirmé prioritaire par l'utilisateur cette session**, avec activation du retrait hors-ligne (jusqu'ici exclu depuis l'Epic 1, le solde n'étant pas vérifiable sans réseau).

**Référence de départ, puis écartée sur demande explicite** : `pos_api/frontend/` a un mécanisme quasi identique pour son propre module tontine ("Sabotage") — cache SQLite (`local_db_service.dart`, `offline_cache_service.dart`), consulté en priorité par les repositories sur Android. L'utilisateur a demandé de **ne pas s'appuyer sur ce module précis** comme référence de conception ; le schéma/les entités mises en cache ci-dessous sont construits directement à partir des modèles Sabotay existants, pas copiés depuis pos_api. Le principe général du cache-first (repository → cache local → réseau seulement si cache vide) reste inspiré de l'architecture pos_api pour ses données produits/clients (non-Sabotage), déjà validée.

### Backend — un seul ajout
`GET /comptes` (`backend/app/api/v1/endpoints/comptes.py`) : registre tenant-wide (scopé agent comme `GET /clients`) avec le **solde calculé en masse** — `crud/transaction.py::get_soldes_bulk()` (requête `GROUP BY compte_id` unique) plutôt qu'un aller-retour `/comptes/{id}/solde` par compte, critique pour un warm-up de cache en zone de faible connectivité. `get_solde()` (mono-compte, existant) refactorisé pour réutiliser la même agrégation (`calculer_champs_solde()`, extrait pour ne pas dupliquer la formule dette/jours manqués) — non-régression vérifiée en comparant les deux endpoints sur le même compte (résultats identiques).

### Mobile — nouvelle couche de cache (`mobile/lib/core/storage/`)
- `LocalDbService` (`sqflite`) : 5 tables (`clients`, `comptes_sabotay` avec solde inline, `transactions_cache`, `entreprise_profil` singleton, `sync_meta`). Montants stockés en TEXT (pas REAL) — reflète tel quel la chaîne Decimal du backend, sans perte de précision supplémentaire par rapport à ce que fait déjà le reste de l'app (`num.parse` partout, jamais de Decimal côté Dart).
- `OfflineCacheService.syncAll()` : sync clients + comptes (paginé) + transactions (paginé, plafonné à 10 pages/2000 lignes — les plus récentes, le registre `GET /transactions` étant trié par date décroissante) + profil, en parallèle (`Future.wait`). Ne lève jamais d'exception (best-effort). Réutilise `GET /transactions` tel quel (déjà construit cette session pour l'onglet web "Transactions") pour l'historique — `Transaction.fromJson` ignore simplement les champs en trop (`client_nom`/`compte_numero`), pas besoin d'un modèle mobile dédié.
- Déclenché : au login (`AuthController.login()`, en tâche de fond, ne bloque pas le retour) et aux mêmes déclencheurs que la file d'écriture existante (`OfflineDrainScope` — retour au premier plan, timer 2 min, retour réseau), pas de nouveau mécanisme de timer.
- **Vidage complet du cache sur changement réel de tenant** (`LocalDbService.viderTout()`), dans la même branche de `AuthController.login()` qui invalide déjà les providers Riverpod scoped-entreprise (Epic 21) — sans quoi l'agent B verrait les données de l'agent A jusqu'au prochain cycle de sync.
- Les 4 repositories (`ClientRepository`, `CompteRepository`, `TransactionRepository`, `EntrepriseRepository`) lisent désormais le cache en priorité, ne retombent sur le réseau que si le cache est vide (premier lancement, avant le tout premier sync).
- **Retrait activé hors-ligne** (`TransactionRepository.createRetrait`) : même mécanisme de mise en file que la collecte (`RetraitResult`/`RetraitQueued`, miroir de `CollecteResult`/`CollecteQueued`). La vérification de solde côté client (`retrait_sheet.dart`, déjà en place avant cette session) fonctionne automatiquement contre le solde caché une fois `getSolde()` cache-first — aucun changement de logique de validation nécessaire, seulement de source de données. La validation qui fait foi reste côté serveur au moment du drain ; un retrait devenu invalide entre-temps (ex. deux appareils) suit le chemin déjà existant (retry puis `dropped`, agent prévenu).
- Indicateur de fraîcheur (`accueil_screen.dart`) : "Données synchronisées il y a X min", basé sur la plus ancienne des 4 dates de `sync_meta` (la moins fraîche des quatre, pour rester honnête).
